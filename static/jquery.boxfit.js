  ...
  var boxfit = function ($nodes, options) {
    return $nodes.each(function () {
      ...
      if (!original_width || !original_height) {
        ...
        inner_span.css('font-size', 100);
        
        var l = (settings.minimum_font_size)?settings.minimum_font_size:1;
        var r = (settings.maximum_font_size)?settings.maximum_font_size:100;
        var m;
        while (r >= l) {
          m = l + Math.floor((r - l) / 2);
          inner_span.css('font-size', m);
          if (m==l) break;
          if ($(this).width() <= original_width && $(this).height() <= original_height) l = m + 1;
          else r = m - 1;
        }
        inner_span.css('font-size', m-2);
